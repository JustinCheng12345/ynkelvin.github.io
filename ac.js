...
function genFlight(time){
    ...
    flight = { acid: random(callsign) + Math.floor(Math.random() * 10000), 
        dep: dep, 
        dest: dest, 
        in_fix: "SIERA", 
        out_fix: out_fix, 
...
function cloak(acid, side, bay, fix){
    flights.find((o, i) => {
        if (o.acid === acid) {
...
if (transferModal) {
  transferModal.addEventListener('show.bs.modal', event => {
    ...
    const modalTitle = transferModal.querySelector('.modal-title')
    const modalBodyInput = transferModal.querySelector('.modal-body input')

    modalTitle.textContent = `Update Transfer ${recipient}`
...
function drawCloak(bay, fix, flight, efs) {
    efs_html = `<div class="row">`;
    if (flight.cloak===""){
        efs_html += `<div class="col-1" onclick="cloak('${flight.acid}', 'L', '${bay}', '${fix}')"></div>`;
    } else if (flight.cloak==="R"){
        efs_html += `<div class="col-2" onclick="cloak('${flight.acid}', '', '${bay}', '${fix}')"></div>`;
    }

    efs_html += efs;

    if (flight.cloak===""){
        efs_html += `<div class="col-1" onclick="cloak('${flight.acid}', 'R', '${bay}', '${fix}')"></div>`;
...
function drawIB(bay, fix, flight) {
    efs_html = `
        <div class="col-10">
            <div class="card border-info border-3 rounded-0">
                <div class="card-body">
                    <div class="container">
                        <div class="row">
                            <div class="col-3 border border-black fs-5">${flight.acid}</div>
                            <div class="col-2 border border-black fs-5 bg-opacity-50 ${flight.fl_light}" onclick="lightup('${flight.acid}', '${bay}', '${fix}')">${flight.fl}</div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-2 border border-black"><button type="button" class="btn efs-btn" data-bs-toggle="modal" data-bs-target="#transferModal" data-bs-whatever="@getbootstrap">${showTime(flight.fix_est)}</button></div>
    ...
    return drawCloak(bay, fix, flight, efs_html);
}

function drawOVF(bay, fix, flight) {
    efs_html = `
        <div class="col-10">
...
function drawBoard(bay, fix) {
    ...
    while (i < flight_draw.length) {
        if (flight_draw[i].dest === "VHHH"){
            efs_html += drawIB(bay, fix, flight_draw[i]);
        } else {
            efs_html += drawOVF(bay, fix, flight_draw[i]);
        }
        i++;
    }
    $(bay).html(efs_html);
